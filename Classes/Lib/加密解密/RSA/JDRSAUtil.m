...
#pragma mark -- 私钥签名/公钥验签
//私钥签名
+ (NSString *)signedStrWith:(NSString *)toSignStr useSecKeyRef:(SecKeyRef)secKey {

    NSData *sha1dg = [toSignStr dataUsingEncoding:NSUTF8StringEncoding];
    
    OSStatus ret;
    size_t siglen = SecKeyGetBlockSize(secKey);
    uint8_t *sig = malloc(siglen);
    bzero(sig, siglen);
    ret = SecKeyRawSign(secKey, kSecPaddingPKCS1SHA1, sha1dg.bytes, sha1dg.length, sig, &siglen);
    if (ret!=errSecSuccess) {
        return nil;
    }
    NSData *eData = [NSData dataWithBytes:sig length:siglen];
    NSData* encodeData = [GTMBase64 encodeData:eData];
    NSString *signatureResult=[[NSString alloc] initWithData:encodeData encoding:NSUTF8StringEncoding];
    
    NSString *baseStr_GTM = [self encodeBase64Data:eData];
    NSString *baseStr = [eData base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength];
    NSLog(@"*****************\nGTMBase:%@\n*****************", baseStr_GTM);
    NSLog(@"*****************\niOSCode:%@\n*****************", baseStr);
...
+ (BOOL)verifySignedStr:(NSString *)signedStr withSourceInfo:(NSString *)sourceStr andSecKeyRef:(SecKeyRef)secKey {
    
    NSData *dataSource = [sourceStr dataUsingEncoding:NSUTF8StringEncoding];
    NSData *encodeData = [signedStr dataUsingEncoding:NSUTF8StringEncoding];
//    NSData *dataSigned = [self decodeBase64Data:encodeData];
    NSData* dataSigned = [GTMBase64 decodeData:encodeData];
    OSStatus ret = SecKeyRawVerify(secKey, kSecPaddingPKCS1SHA1, dataSource.bytes, dataSource.length, dataSigned.bytes, dataSigned.length);
