...
static DQEaseMobCallManager *callManager = nil;

@interface DQEaseMobCallManager()<EMChatManagerDelegate, EMCallManagerDelegate, EMCallBuilderDelegate>
...
@implementation DQEaseMobCallManager
...
+ (instancetype)sharedManager
{
    ...
    dispatch_once(&onceToken, ^{
        callManager = [[DQEaseMobCallManager alloc] init];
        NSLog(@"=======DQEaseMobCallManager is created========");
    });
...
- (void)dealloc {
    ...
    NSLog(@"=======DQEaseMobCallManager is dealloc========");
}
    ...
    @synchronized (_callLock) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self.currentController) {
                [[UIApplication sharedApplication].activityViewController  presentViewController:self.currentController animated:NO completion:nil];
            }
        });
    }
}
...
- (void)makeCallWithUsername:(NSString *)aUsername type:(EMCallType)aType {
    ...
    void (^completionBlock)(EMCallSession *, EMError *) = ^(EMCallSession *aCallSession, EMError *aError){
        DQEaseMobCallManager *strongSelf = weakSelf;
        if (strongSelf) {
